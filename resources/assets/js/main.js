...
AWS.config.credentials.get(function(){
    ...
    apigClient.invokeApi(params, pathTemplate, method, additionalParams, body)
        .then(function(result){
            ...
            var html = fs.readFileSync(__dirname + '/card.html', 'utf8');

            for(var i = 0; i <= result.data.tracks.length-1; i++) {
                var mp3Item = result.data.tracks[i];

                var dom = domify(html);
                dom.querySelector(".card-title").textContent = mp3Item.Artist;
                ...
                dom.querySelector(".mp3description").textContent = mp3Item.Description;
                ...
                var origUrl = "https://doyvfpldm7c3l.cloudfront.net/Thumbnails/" + mp3Item.IconUrl;

                // Generating a signed URL
                var signedUrl = cfsign.getSignedUrl( origUrl, signingParams);

                dom.querySelector(".mp3image").src = signedUrl;
